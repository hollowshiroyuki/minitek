...
{
    c->u = !floor_get_tile(floor, (sfVector2i){pos.x, pos.y - 1}).sand;
    c->d = !floor_get_tile(floor, (sfVector2i){pos.x, pos.y + 1}).sand;
    c->l = !floor_get_tile(floor, (sfVector2i){pos.x - 1, pos.y}).sand;
    c->r = !floor_get_tile(floor, (sfVector2i){pos.x + 1, pos.y}).sand;
}
...
static void top(connec_t *c, screen_t *scr, sfVector2i pos)
{
    ...
    if (!c->u && !c->l) {
        ...
        tex = (c->l ? 6 * 32 + 6 : 6 * 32 + 7) + (c->u ? 0 : 1) * 32;
    ...
    if (!c->u && !c->r) {
        ...
        tex = (c->r ? 6 * 32 + 8 : 6 * 32 + 7) + (c->u ? 0 : 1) * 32;
        screen_render_tile(scr, pos, tex, 0);
    }
}
...
static void bottom(connec_t *c, screen_t *scr, sfVector2i pos)
{
    ...
    if (!c->d && !c->l) {
        ...
        tex = (c->l ? 6 * 32 + 6 : 6 * 32 + 7) + (c->d ? 2 : 1) * 32;
        screen_render_tile(scr, pos, tex, 0);
    }
    pos.x += 8;
    if (!c->d && !c->r) {
        ...
        tex = (c->r ? 6 * 32 + 8 : 6 * 32 + 7) + (c->d ? 2 : 1) * 32;
        screen_render_tile(scr, pos, tex, 0);
    }
}
