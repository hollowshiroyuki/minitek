...
You're currently reading the README of the Windows version, for the Linux see [here](https://github.com/hollowshiroyuki/minitek/blob/master/README.md).

Just go to the [releases](https://github.com/hollowshiroyuki/minitek/releases/) section and download the windows build.

#### Build It !

If you want to build it yourself you'll only need `mingw64` and `make` to compile it.

Clone the repo.

Open the folder in `powershell` or `cmd`.

Type `make` and `Enter`.

𝓟𝓻𝓸𝓰𝓻𝓪𝓶𝓶𝓲𝓷𝓰 𝓶𝓪𝓰𝓲𝓬 :tm:.

And done !

(Don't forget to get the `.dll`s on the CSFML website to launch it.)
